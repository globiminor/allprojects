...
namespace OCourse.Commands
{
	public class CmdCourseTransfer : IDisposable
	{
		...
		private OcadWriter Writer
		{
			get
			{
				if (_writer == null)
				{
					...
					{
						System.IO.Directory.CreateDirectory(System.IO.Path.GetDirectoryName(_exportFile));
		...
		public void Export(Course course, int climb)
		{
			...
			Dictionary<string, Element> objStrMapElements = new Dictionary<string, Element>();
			foreach (var mapElem in Reader.EnumMapElements())
			{
				if (!string.IsNullOrWhiteSpace(mapElem.ObjectString))
				{
					objStrMapElements.Add(mapElem.ObjectString, mapElem);
				}
			}
			foreach (SimpleSection sec in course.GetAllSections())
			{
				foreach (Control ctr in new[] { sec.From, sec.To })
				{
					...
					{
						if (!ObjStrMapElements.TryGetValue(ctr.Element.ObjectString, out Element mapElem))
						{ mapElem = ctr.Element; }
						Writer.Append(ctr, mapElem);
		...
		private Dictionary<string, Element> ObjStrMapElements => _objStrMapElements ?? (_objStrMapElements = GetObjStrMapElements());
		private Dictionary<string, Element> _objStrMapElements;
		private Dictionary<string, Element> GetObjStrMapElements()
		{
			Dictionary<string, Element> objStrMapElements = new Dictionary<string, Element>();
			foreach (var mapElem in Reader.EnumMapElements())
			{
