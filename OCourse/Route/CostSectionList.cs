...
namespace OCourse.Route
{
	public class CostSectionlist : CostBase, IInfo, ICostSectionlists
	{
			...
			RouteCalculator routeCalc, double resol)
		{
			...
			{
				foreach (var sectionList in GetCostSectionLists(permut, routeCalc, resol))
				{
					yield return sectionList;
				}
			}
		}

		public static IEnumerable<CostSectionlist> GetCostSectionLists(
			ViewModels.PermutationVm permutation,
			RouteCalculator routeCalc, double resol)
		{
			int iLeg = 0;
			string sLeg = string.Empty;
			SectionList sections = null;
			foreach (var next in permutation.Permutation.NextControls)
			{
				if (next.Control == SectionList.LegStart)
				{
					if (sections != null)
					{
						...
						cost.Name = $"{permutation.StartNr}{sLeg}";
						yield return cost;
						sLeg = $".{iLeg + 1}";
					}
					sections = null;
