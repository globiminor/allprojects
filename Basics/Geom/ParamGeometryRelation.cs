...
namespace Basics.Geom
{
	public class ParamGeometryRelation
	{
		...
		private class Info
		{
			...
			public int CompareTo(Info other)
			{
				...
				while (true)
				{
					...
					if (i != 0)
					{
						if (IsEqualPolylinePoint(other, thisParent, otherParent))
						{
							return 0;
						}
			...
			private bool IsEqualPolylinePoint(Info other, LinkedListNode<ParentGeometry> thisParent, LinkedListNode<ParentGeometry> otherParent)
			{
				if (thisParent.Value.Geometry != otherParent.Value.Geometry) return false;
				if (!(thisParent.Value.Geometry is ISimplePolyline line)) return false;
				if (thisParent.Previous != null || otherParent.Previous != null) return false;
				if (Param == null || other.Param == null) return false;

				if (thisParent.Value.ChildIndex + Param.X == otherParent.Value.ChildIndex + other.Param.X)
				{ return true; }
				// TODO
				if (Param.X == 0 && thisParent.Value.ChildIndex == 0 &&
					other.Param.X == 1 && line.Points.Count - 1 == otherParent.Value.ChildIndex)
				{ throw new NotImplementedException(); }
				if (other.Param.X == 0 && otherParent.Value.ChildIndex == 0 &&
					Param.X == 1 && line.Points.Count - 1 == thisParent.Value.ChildIndex)
				{ throw new NotImplementedException(); }
		...
		public override string ToString() => $"{Intersection};x:{_x};y:{_y}";
		...
		/// 
		/// </summary>
		/// <returns>same instance, with parent add</returns>
		public ParamGeometryRelation AddParent(IGeometry parent, int childIndex, IGeometry child)
		{
		...
		public class GeomComparer : IEqualityComparer<ParamGeometryRelation>
		{
			private readonly IGeometry _parent;
			private readonly bool _nullFirst;
			private readonly PointComparer _pointCmp;
			public GeomComparer(IGeometry parentGeometry, bool nullFirst = false)
			{
				_parent = parentGeometry;
				_nullFirst = nullFirst;
				_pointCmp = new PointComparer();
			}
			...
			public bool Equals(ParamGeometryRelation x, ParamGeometryRelation y)
			{
				...
				Info xInfo = x.GetInfo(_parent);
				Info yInfo = y.GetInfo(_parent);

				LinkedListNode<ParentGeometry> xLast = xInfo.Hierarchy.Last;
				LinkedListNode<ParentGeometry> yLast = yInfo.Hierarchy.Last;
				IGeometry xGeom = xLast.Value.Geometry;
				if (xGeom != yLast.Value.Geometry)
				{ return false; }

				if (xGeom is ISimplePolyline line)
				{
					Info i0;
					Info i1;
					LinkedListNode<ParentGeometry> n0;
					LinkedListNode<ParentGeometry> n1;
					...
					if (n1.Value.ChildIndex + 1 == n0.Value.ChildIndex)
					{ return true; }

					if (n0.Value.ChildIndex == 0 && n1.Value.ChildIndex + 1 == line.Points.Count)
					{ return true; }
				}
			...
			public int GetHashCode(ParamGeometryRelation obj)
			{
				if (obj.Intersection is IPoint p)
				{ return _pointCmp.GetHashCode(p); }

				Info iParent = obj.GetInfo(_parent);
				int hashCode = iParent?.Hierarchy?.Last.Value.Geometry.GetHashCode() ?? 0;
				return hashCode;
			}
		}
		...
		public static List<ParamGeometryRelation> Assemble(IGeometry x, IGeometry y, IList<ParamGeometryRelation> borderRelations)
		{
			...
			List<ParamGeometryRelation> sortY = borderRelations != null
				? new List<ParamGeometryRelation>(borderRelations)
				: new List<ParamGeometryRelation>(0);
			Sort(sortY, y);
