...
namespace LeastCostPathUI
{
  public partial class CntOutGrid : UserControl
  {
    ...
    public void Export(DoubleBaseGrid costGrid, IntGrid dirGrid, Steps step)
    {
      ...
      {
        if (CostGrid != null)
        { costGrid.Save(CostGrid); }
        if (CostImage != null)
        { ImageGrid.GridToTif(costGrid.ToIntGrid() % 256, CostImage, r, g, b); }
      }
      if (dirGrid != null)
      {
        if (DirGrid != null)
        { dirGrid.Save(DirGrid); }
        if (DirImage != null)
        {
          // make sure that the start point cell returns a valid value for the step angle array
          IntGrid angleGrid = ((step[(dirGrid - 1).Abs() % step.Count] / Math.PI + 1.0) * 128).ToIntGrid();
